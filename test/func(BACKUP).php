...
function post_params($page,$link){
	...
	if(count($arr)>0){
		...
		for ($i=0;$i<count($arr);$i++){
			$query_set.="post_".$arr[$i]->NodeValue;
			if(isset($arr[$i+1]))$query_set.="&";
		}
		die($query_set);
		$query_res=mysqli_query($mysqli_link,"SELECT IF( EXISTS(SELECT * FROM param_set WHERE page_id=(select id from pages where page_name='".$url['path']."' and proj_id={$proj_id['id']} )), 1, 0) as ch");
		$q=mysqli_fetch_assoc($query_res);
		if($q['ch']==0){
			$set=mysqli_query($mysqli_link,"insert into param_set values(null,{$proj_id['id']},(select id from pages where page_name='".$url['path']."' and proj_id={$proj_id['id']},'{$query_set}')");

		} else{
			$set=mysqli_query($mysqli_link,"SELECT * FROM param_set WHERE page_id=(select id from pages where page_name='".$url['path']."' and proj_id={$proj_id['id']} )");
			$set=mysqli_fetch_assoc($set);
			$new_set=$set['par_set']."|".$query_set;
			die($new_set);
			$update_set=mysqli_query($mysqli_link,"update param_set set par_set = '{$new_set}' where par_set = '{$set['par_set']}';");
			die();
		}
	}
	//return $arr;
		...
		if(isset($url['host'])){
			if(domain==$url['host']){
				if(isset($url['path'])){
	              	...
	              	if(isset($url['query']))$link=$url['host']."index.php?".$url['query'];
	                else $link=$url['host'];
	            }
			}else continue 1;
...
/* WRITE TO PAGES TABLE */
				...
				if($page_test['pg']==0){
					...
					$query_res=mysqli_query($mysqli_link,"insert into `pages` values(null,{$proj_id['id']},'{$url['path']}');");
				}
